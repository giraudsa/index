...
public abstract class Node1D<K extends Comparable<K>, V> extends AbstractNode<K, V> implements Iterable<V> {
	...
	protected class ValueNodeIterator implements Iterator<V> {
		...
		private void searchNext() throws IOException, SerializationException {
			while (current != null) {
				nodesStack.push(current);
				if (min != null) {
	...
	private static final <K extends Comparable<K>, V> int height(final Node1D<K, V> node, final CacheModifications modifs) throws IOException, SerializationException {
	...
	// fake noeud
	protected Node1D(final Class<K> keyType, final Class<V> valueType, final AbstractIndex<?, ?, ?> index, final CacheModifications modifs) {
	 ...
	 * @param valueType
	 */
	protected Node1D(final long position, final AbstractIndex<?, ?, ?> index, final Class<K> keyType, final Class<V> valueType) {
	 ...
	 */
	protected Node1D(final long keyPosition, final Long valuePosition, final AbstractIndex<?, ?, ?> index, final Class<K> keyType, final Class<V> valueType, final CacheModifications modifs) throws SerializationException { // a ecrire sur le disque
	...
	private Node1D<K, V> balance(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
		updateHeight(modifs);
	...
	private Node1D<K, V> deleteLeft(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
	...
	private AbstractNode<K, V> deleteNode(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
		final Node1D<K, V> r = getRight(modifs);
		final Node1D<K, V> l = getLeft(modifs);
		if (r == null && l == null)
			return null;
	...
	private long leftPositionPosition() {
		return heightPosition() + Integer.BYTES;
	}

	private Node1D<K, V> leftRotation(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
	...
	private void nodeInit(final long keyPosition, final Long valuePosition, final CacheModifications modifs) throws SerializationException {
		isFake = false;
	...
	private long rightPositionPosition() {
		return leftPositionPosition() + Long.BYTES;
	}

	private Node1D<K, V> rightRotation(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
	...
	private void updateHeight(final CacheModifications modifs) throws StorageException, IOException, SerializationException {
		setHeight(Math.max(height(getRight(modifs), modifs), height(getLeft(modifs), modifs)) + 1, modifs);
	}

	protected boolean containKey(final K key, final CacheModifications modifs) throws IOException, StorageException, SerializationException {
		return findNode(key, modifs) != null;
	}

	@Override
	protected AbstractNode<K, V> findNode(final K key, final CacheModifications modifs) throws IOException, StorageException, SerializationException {
		if (this.isFake())
			return null;
	...
	protected Node1D<K, V> getLeft(final CacheModifications modifs) throws IOException, SerializationException {
		return index.getStuff(leftPosition(modifs), this.getClass(), modifs);
	}

	@SuppressWarnings("unchecked")
	protected Node1D<K, V> getRight(final CacheModifications modifs) throws IOException, SerializationException {
		return index.getStuff(rightPosition(modifs), this.getClass(), modifs);
	}

	protected int height(final CacheModifications modifs) throws IOException, SerializationException {
	...
	protected long leftPosition(final CacheModifications modifs) throws IOException, SerializationException {
	...
	protected long rightPosition(final CacheModifications modifs) throws IOException, SerializationException {
		return index.getStuff(rightPositionPosition(), Long.class, modifs);
	}

	protected void setHeight(final int height, final CacheModifications modifs) throws IOException, SerializationException {
	...
	@Override
	AbstractNode<K, V> addAndBalance(final K key, final long keyPosition, final Long valuePosition, final CacheModifications modifs) throws IOException, StorageException, SerializationException {
		if (this.isFake()) {
			nodeInit(keyPosition, valuePosition, modifs);
			return this;
		}
	...
	@Override
	AbstractNode<K, V> deleteAndBalance(final K key, final CacheModifications modifs) throws IOException, StorageException, SerializationException {
		if (this.isFake())
			return this;
		final AbstractNode<K, V> r = getRight(modifs);
		final AbstractNode<K, V> l = getLeft(modifs);
