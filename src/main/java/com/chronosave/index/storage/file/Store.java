...
public class Store<U> {
	...
	Store(final Path basePath, final Class<U> clazz, final SerializationStore marshaller, final IdManager idManager, final long lastGoodVersion) throws IOException, StorageException, ClassNotFoundException, SerializationException, StoreException {
		...
		if (filePrimary != null) {// file read
			primaryIndex = new PrimaryIndexFile<>(filePrimary, this);
			if (data.getVersion() > lastGoodVersion || primaryIndex.getVersion() != data.getVersion())
	...
	private void createIndex(final ComputeKey<?, U> computeKey) throws IOException, StorageException, SerializationException {
	...
	private <K extends Comparable<K>> void createIndexKeyToMultiId(final ComputeKey<K, U> computeKey) throws IOException, StorageException, SerializationException {
		index.put(computeKey, new IndexKeyToMultiId<>(basePath, computeKey.getKeyType(), this, computeKey));
	}

	private <K extends Comparable<K>> void createIndexMultiKeyToMultiId(final ComputeKey<K, U> computeKey) throws IOException, StorageException, SerializationException {
		index.put(computeKey, new IndexMultiKeyToMultiId<>(basePath, computeKey.getKeyType(), this, computeKey));
	}

	private <K extends List<Double>> void createIndexSpatial(final ComputeKey<K, U> computeKey) throws IOException, StorageException, SerializationException {
	...
	protected void delete(final Collection<String> ids, final long v, final Collection<CacheModifications> modifs) throws IOException, StorageException, SerializationException {
	...
	protected CloseableIterator<U> getAllObjectsWithMaxVersionLessThan(final long version) throws IOException, StorageException, SerializationException {
	...
	protected <K> CloseableIterator<String> selectIdFromClassWhere(final AbstractCondition<K, U> condition, final ReadWriteLock locker) throws IOException, StorageException, SerializationException, StoreException, InterruptedException {
	...
	protected void store(final Collection<U> v, final long version, final Collection<CacheModifications> modifs) throws StorageException, IOException, SerializationException {
		modifs.add(primaryIndex.stockpile(v, version));
		for (final AbstractIndex<U, ?, ?> indexe : index.values())
			modifs.add(indexe.stockpile(v, version));
	}

	protected Long writeData(final U object, final long version) throws IOException, SerializationException, StorageException {
