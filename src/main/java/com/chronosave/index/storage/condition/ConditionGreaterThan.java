...
public class ConditionGreaterThan<K extends Comparable<K>, U> extends ConditionCompare<K, U> {
	...
	public ConditionGreaterThan(final Class<U> typeObjet, final Class<K> typeReturn,
	...
	public CloseableIterator<String> run(final AbstractIndex<U, ?, ?> index, final ReadWriteLock locker)
		...
		try {
			return ((Index1D<U, K, String, ?>) index).getBetween(valeur, null, locker);
	...
	public <V> Collection<U> runMemory(final MemoryIndex<?, V> memoryIndex, final ReadWriteLock locker)
		...
		locker.lockRead();
		final Collection<U> ret = (Collection<U>) ((MemoryIndexAVL<K, V>) memoryIndex).getBetween(valeur, null);
		locker.unlockRead();
		return ret;
	}
